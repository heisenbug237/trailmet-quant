...
def imagenet_get_datasets(data_dir, arch, load_train=True, load_test=True):
    ...
    if arch in ['inception_v3', 'googlenet', 'inceptionv3', 'inceptionv4', 'inceptionresnetv2']:
        resize, crop = 336, 299
    else:
        resize, crop = 256, 224
    if arch == 'googlenet':
...
def get_data_loaders(datasets_fn, data_dir, batch_size, num_workers, validation_split=0.1,
